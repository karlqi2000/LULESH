...
template <int type>
void SendPlane(Real_t *destAddr, Real_t *srcAddr, Index_t sendCount, Index_t dx,
               Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
...
template <int type>
void AddPlane(Real_t *srcAddr, Real_t *destAddr, Index_t recvCount, Index_t dx,
              Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
  int tid = item_ct1.get_local_id(2) +
            item_ct1.get_group(2) * item_ct1.get_local_range().get(2);
...
template <int type>
void CopyPlane(Real_t *srcAddr, Real_t *destAddr, Index_t recvCount, Index_t dx,
               Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
  int tid = item_ct1.get_local_id(2) +
            item_ct1.get_group(2) * item_ct1.get_local_range().get(2);
  if (tid >= recvCount) return;
...
template <int type>
void SendEdge(Real_t *destAddr, Real_t *srcAddr, Index_t sendCount, Index_t dx,
              Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
...
template <int type>
void AddEdge(Real_t *srcAddr, Real_t *destAddr, Index_t recvCount, Index_t dx,
             Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
  int i = item_ct1.get_local_id(2) +
          item_ct1.get_group(2) * item_ct1.get_local_range().get(2);
...
template <int type>
void CopyEdge(Real_t *srcAddr, Real_t *destAddr, Index_t recvCount, Index_t dx,
              Index_t dy, Index_t dz, sycl::nd_item<3> item_ct1) {
  int i = item_ct1.get_local_id(2) +
          item_ct1.get_group(2) * item_ct1.get_local_range().get(2);
  if (i >= recvCount) return;
...
void AddCorner(Real_t *destAddr, Real_t src) { destAddr[0] += src; }

void CopyCorner(Real_t *destAddr, Real_t src) { destAddr[0] = src; }

/******************************************/

void CommSendGpu(Domain &domain, int msgType, Index_t xferFields,
    ...
    if (planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        ...
        stream->submit([&](sycl::handler &cgh) {
          auto domain_src_ct1 = &(domain.*src)(0);

          cgh.parallel_for(
              sycl::nd_range<3>(
                  sycl::range<3>(1, 1, (sendCount + block - 1) / block) *
                      sycl::range<3>(1, 1, block),
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<0>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
      ...
      stream->memcpy(destAddr, d_destAddr,
                     xferFields * sendCount * sizeof(Real_t));
      stream->wait();
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<1>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += sendCount;
  ...
  if (rowMin | rowMax) {
    ...
    if (rowMin) {
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<2>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += sendCount;
      ...
      MPI_Isend(destAddr, xferFields * sendCount, baseType,
                myRank - domain.tp(), msgType, MPI_COMM_WORLD,
                &domain.sendRequest[pmsg]);
      ++pmsg;
    }
    if (rowMax && doSend) {
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<3>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += sendCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<4>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += sendCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendPlane<5>(d_destAddr, domain_src_ct1, sendCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += sendCount;
  ...
  if (!planeOnly) {
    if (rowMin && colMin) {
      ...
      d_destAddr =
          &domain.d_commDataSend[pmsg * maxPlaneComm + emsg * maxEdgeComm];
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member src = fieldData[fi];
        stream->submit([&](sycl::handler &cgh) {
          auto domain_src_ct1 = &(domain.*src)(0);
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                                    sycl::range<3>(1, 1, block),
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<0>(d_destAddr, domain_src_ct1, dz, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dz;
      ...
      stream->memcpy(destAddr, d_destAddr, xferFields * dz * sizeof(Real_t));
      stream->wait();
    ...
    if (rowMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                                    sycl::range<3>(1, 1, block),
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<1>(d_destAddr, domain_src_ct1, dx, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dx;
      ...
      stream->memcpy(destAddr, d_destAddr, xferFields * dx * sizeof(Real_t));
      stream->wait();
    ...
    if (colMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                                    sycl::range<3>(1, 1, block),
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<2>(d_destAddr, domain_src_ct1, dy, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dy;
      ...
      stream->memcpy(destAddr, d_destAddr, xferFields * dy * sizeof(Real_t));
      stream->wait();
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<3>(d_destAddr, domain_src_ct1, dz, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dz;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<4>(d_destAddr, domain_src_ct1, dx, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dx;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<5>(d_destAddr, domain_src_ct1, dy, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dy;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<6>(d_destAddr, domain_src_ct1, dz, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dz;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<7>(d_destAddr, domain_src_ct1, dx, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dx;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<8>(d_destAddr, domain_src_ct1, dy, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dy;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<9>(d_destAddr, domain_src_ct1, dz, dx, dy, dz,
                            item_ct1);
              });
        });
        d_destAddr += dz;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<10>(d_destAddr, domain_src_ct1, dx, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += dx;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                                ...
                                sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                SendEdge<11>(d_destAddr, domain_src_ct1, dy, dx, dy, dz,
                             item_ct1);
              });
        });
        d_destAddr += dy;
    ...
    if (rowMin && colMin && planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        stream->memcpy(&comBuf[fi], &(domain.*fieldData[fi])(0),
                       sizeof(Real_t));
      }
      stream->wait();
    ...
    if (rowMin && colMin && planeMax && doSend) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        stream->memcpy(&comBuf[fi], &(domain.*fieldData[fi])(idx),
                       sizeof(Real_t));
      }
      stream->wait();
      MPI_Isend(comBuf, xferFields, baseType, toRank, msgType, MPI_COMM_WORLD,
...
void CommSBNGpu(Domain &domain, int xferFields, Domain_member *fieldData,
                sycl::queue **streams) {
  if (domain.numRanks() <= 1) return;
  ...
  if (planeMin | planeMax) {
    ...
    if (planeMin) {
      ...
      stream->memcpy(d_srcAddr, srcAddr, xferFields * opCount * sizeof(Real_t));
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        stream->submit([&](sycl::handler &cgh) {
          auto domain_dest_ct1 = &(domain.*dest)(0);

          cgh.parallel_for(
              sycl::nd_range<3>(
                  sycl::range<3>(1, 1, (opCount + block - 1) / block) *
                      sycl::range<3>(1, 1, block),
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<0>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<1>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<2>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<3>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<4>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                AddPlane<5>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                            item_ct1);
              });
        });
        d_srcAddr += opCount;
  ...
  if (rowMin & colMin) {
    ...
    d_srcAddr =
        &domain.d_commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm];
    MPI_Wait(&domain.recvRequest[pmsg + emsg], &status);
    stream->memcpy(d_srcAddr, srcAddr, xferFields * dz * sizeof(Real_t));
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      stream->submit([&](sycl::handler &cgh) {
        auto domain_dest_ct1 = &(domain.*dest)(0);
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<0>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
  ...
  if (rowMin & planeMin) {
    ...
    d_srcAddr =
        &domain.d_commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm];
    MPI_Wait(&domain.recvRequest[pmsg + emsg], &status);
    stream->memcpy(d_srcAddr, srcAddr, xferFields * dx * sizeof(Real_t));
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      stream->submit([&](sycl::handler &cgh) {
        auto domain_dest_ct1 = &(domain.*dest)(0);
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<1>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
  ...
  if (colMin & planeMin) {
    ...
    d_srcAddr =
        &domain.d_commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm];
    MPI_Wait(&domain.recvRequest[pmsg + emsg], &status);
    stream->memcpy(d_srcAddr, srcAddr, xferFields * dy * sizeof(Real_t));
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      Domain_member dest = fieldData[fi];
      stream->submit([&](sycl::handler &cgh) {
        auto domain_dest_ct1 = &(domain.*dest)(0);
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<2>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<3>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<4>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<5>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<6>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<7>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<8>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<9>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<10>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              AddEdge<11>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
  ...
  if (rowMin & colMin & planeMin) {
    ...
    /* corner at domain logical coord (0, 0, 0) */
    Real_t *comBuf =
        &domain.commDataRecv[pmsg * maxPlaneComm + emsg * maxEdgeComm +
                             cmsg * CACHE_COHERENCE_PAD_REAL];
    MPI_Wait(&domain.recvRequest[pmsg + emsg + cmsg], &status);
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      stream->submit([&](sycl::handler &cgh) {
        auto domain_fieldData_fi_ct0 = &(domain.*fieldData[fi])(0);
        auto comBuf_fi_ct1 = comBuf[fi];

        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
            [=](sycl::nd_item<3> item_ct1) {
              AddCorner(domain_fieldData_fi_ct0, comBuf_fi_ct1);
            });
      });
    }
    ++cmsg;
  ...
  if (rowMin & colMin & planeMax) {
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      stream->submit([&](sycl::handler &cgh) {
        auto domain_fieldData_fi_idx_ct0 = &(domain.*fieldData[fi])(idx);
        auto comBuf_fi_ct1 = comBuf[fi];

        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
            [=](sycl::nd_item<3> item_ct1) {
              AddCorner(domain_fieldData_fi_idx_ct0, comBuf_fi_ct1);
            });
      });
    }
    ++cmsg;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<0>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<1>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<2>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<3>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<4>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
        ...
        stream->submit([&](sycl::handler &cgh) {
          ...
          cgh.parallel_for(
              sycl::nd_range<3>(
                  ...
                  sycl::range<3>(1, 1, block)),
              [=](sycl::nd_item<3> item_ct1) {
                CopyPlane<5>(d_srcAddr, domain_dest_ct1, opCount, dx, dy, dz,
                             item_ct1);
              });
        });
        d_srcAddr += opCount;
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<0>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<1>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<2>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<3>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<4>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<5>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<6>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<7>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dx;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<8>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dy;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dz + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<9>(d_srcAddr, domain_dest_ct1, dz, dx, dy, dz, item_ct1);
            });
      });
      d_srcAddr += dz;
    }
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dx + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<10>(d_srcAddr, domain_dest_ct1, dx, dx, dy, dz,
                           item_ct1);
            });
      });
      d_srcAddr += dx;
      ...
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, (dy + block - 1) / block) *
                              ...
                              sycl::range<3>(1, 1, block)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyEdge<11>(d_srcAddr, domain_dest_ct1, dy, dx, dy, dz,
                           item_ct1);
            });
      });
      d_srcAddr += dy;
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyCorner(domain_fieldData_fi_ct0, comBuf_fi_ct1);
            });
      });
    }
    ++cmsg;
    ...
    for (Index_t fi = 0; fi < xferFields; ++fi) {
      stream->submit([&](sycl::handler &cgh) {
        ...
        cgh.parallel_for(
            sycl::nd_range<3>(sycl::range<3>(1, 1, 1), sycl::range<3>(1, 1, 1)),
            [=](sycl::nd_item<3> item_ct1) {
              CopyCorner(domain_fieldData_fi_idx_ct0, comBuf_fi_ct1);
            });
      });
    }
    ++cmsg;
...
void CommMonoQGpu(Domain &domain, sycl::queue *stream) {
  ...
  if (planeMin | planeMax) {
    ...
    if (planeMin) {
      ...
      for (Index_t fi = 0; fi < xferFields; ++fi) {
        Domain_member dest = fieldData[fi];
        stream->memcpy(&(domain.*dest)(fieldOffset[fi]), srcAddr,
                       opCount * sizeof(Real_t));
        srcAddr += opCount;
